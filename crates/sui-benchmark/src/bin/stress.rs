...
use test_utils::objects::{
    generate_gas_object, generate_gas_objects, generate_gas_objects_with_owner,
};
...
struct Opts {
    ...
    #[clap(long, default_value = "5", global = true)]
    pub num_transfer_accounts: usize,
...
async fn init_shared_counter_benchmark(
    ...
    // create enough gas
    let mut gas = vec![];
    let mut counters_gas = generate_gas_objects(count);
...
async fn main() {
    ...
    let (payload, addresses, clients) = if let TransactionType::SharedCounter =
        opts.transaction_type
    {
        init_shared_counter_benchmark(max_in_flight_ops, configs).await
    } else if let TransactionType::TransferObject = opts.transaction_type {
        init_object_transfer_benchmark(max_in_flight_ops, opts.num_transfer_accounts, configs).await
    } else {
        unreachable!()
    };
